...
class HarnessTest < Minitest::Test
  ...
  def test_run_filters_issues
    with_workspace(head: Pathname(__dir__).join("data/foo.tgz").to_s) do |workspace|
      processor = Class.new(Processor) do
        def analyze(changes)
          Results::Success.new(guid: guid, analyzer: Analyzer.new(name: "Test", version: "0.1.3")).tap do |result|
            result.add_issue Issues::Identified.new(path: Pathname("test/cli/test_test.rb"),
                                                     location: Location.new(start_line: 0, start_column: nil, end_line: nil, end_column: nil),
                                                     id: "id1")
            result.add_issue Issues::Identified.new(path: Pathname("no_such_file"),
                                                    location: Location.new(start_line: 0, start_column: nil, end_line: nil, end_column: nil),
                                                    id: "id2")
            result.add_issue Issues::Identified.new(path: Pathname("taggeing_test.rb"),
                                                    location: Location.new(start_line: 0, start_column: nil, end_line: nil, end_column: nil),
                                                    id: "id3")
          end
        end
      ...
      result = harness.run
      assert_instance_of Results::Success, result
      assert_equal [Issues::Identified.new(path: Pathname("test/cli/test_test.rb"),
                                           location: Location.new(start_line: 0, start_column: nil, end_line: nil, end_column: nil),
                                           id: "id1")], result.issues
    end
  end
