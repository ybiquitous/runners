class NodeHarness::Processor::Tyscan < NodeHarness::Processor
  ...
  def analyzer
    @analyzer ||= NodeHarness::Analyzer.new(name: "TyScan", version: analyzer_version)
  end
  ...
  def setup
    ensure_runner_config_schema(Schema.runner_config) do |config|
      if !(current_dir + 'tyscan.yml').exist? && config[:config].nil?
        ...
        return NodeHarness::Results::Success.new(guid: guid, analyzer: analyzer)
      end
